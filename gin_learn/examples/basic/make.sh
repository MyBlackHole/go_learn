...
# CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o release/linux/amd64/bin/URun-getData
# CGO_ENABLED=0 GOOS=freebsd GOARCH=amd64 go build -o release/freebsd/amd64/bin
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o release/windows/amd64/go_web.exe
